...
export default async function Page({params}:{params:Promise<{id:string}>}){
  ...
  const [{data:order},{data:items},{data:events},{data:pods}]=await Promise.all([
    ...
    supabase.from('hamper_order_events').select('id,event_type,created_at').eq('order_id',id).order('created_at',{ascending:false}).limit(8),
  ...
  return <Shell><div className="top"><div><div className="eyebrow">{order.priority?'Priority order':'Hamper order'}</div><h1>{order.order_number}</h1></div><div className="top-actions"><a className="secondary-button" href={`/orders/${id}/docket`} target="_blank">Print Docket</a>{order.status==='production_complete'&&<StatusAction orderId={id} action="ready" label="Mark Ready"/>}{order.status==='ready'&&<StatusAction orderId={id} action="delivered" label={order.fulfilment_method==='collection'?'Mark Collected':order.fulfilment_method==='dpd'?'Mark DPD Collected':'Mark Delivered'}/>} {order.status==='delivered'&&<span className="pill">Upload POD to complete</span>}</div></div>
    ...
    <h2 className="section-title">Production</h2><div className="card table-card"><table className="table"><thead><tr><th>Hamper</th><th>Required</th><th>Complete</th><th>Remaining</th><th>Log Production</th></tr></thead><tbody>{(items??[]).map((i:any)=><tr key={i.order_item_id}><td>{i.description_snapshot}</td><td>{i.quantity_required}</td><td>{i.quantity_completed}</td><td>{i.quantity_remaining}</td><td>{i.quantity_remaining>0?<ProductionEntry orderId={id} itemId={i.order_item_id} remaining={i.quantity_remaining}/>:<span className="complete-mark">✓ Complete</span>}</td></tr>)}</tbody></table></div>
