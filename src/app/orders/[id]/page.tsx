...
export default async function Page({params}:{params:Promise<{id:string}>}){
  ...
  return <Shell><div className="top"><div><div className="eyebrow">{order.priority?'Priority order':'Hamper order'}</div><h1>{order.order_number}</h1></div><div className="top-actions"><a className="secondary-button" href={`/orders/${id}/docket`} target="_blank">Print Docket</a>{order.status==='scheduled'&&<StatusAction orderId={id} action="start" label="Start Production" secondary/>}{['scheduled','in_progress'].includes(order.status)&&remaining>0&&<StatusAction orderId={id} action="production-complete" label="Complete Production"/>}{order.status==='production_complete'&&<StatusAction orderId={id} action="ready" label="Mark Ready"/>}{order.status==='ready'&&<StatusAction orderId={id} action="delivered" label={order.fulfilment_method==='collection'?'Mark Collected':order.fulfilment_method==='dpd'?'Mark DPD Collected':'Mark Delivered'}/>} {order.status==='delivered'&&<span className="pill">Upload POD to complete</span>}</div></div>
