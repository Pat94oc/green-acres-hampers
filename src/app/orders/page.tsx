...
export default async function Page(){
  ...
  return <Shell><div className="top"><div><div className="eyebrow">All active and historic orders</div><h1>Orders</h1></div><Link href="/new-order" className="button">+ New Order</Link></div><div className="card table-card"><table className="table"><thead><tr><th>Order</th><th>Customer</th><th>Due</th><th>Status</th><th>Method</th></tr></thead><tbody>{(orders??[]).map((o:any)=><tr key={o.id}><td><Link className="table-link" href={`/orders/${o.id}`}>{o.order_number}</Link>{o.priority&&<span className="pill priority">PRIORITY</span>}</td><td>{o.hamper_customers?.name??'—'}</td><td>{formatDate(o.delivery_date)}</td><td><span className="pill">{statusLabel(o.status)}</span></td><td>{methodLabel(o.fulfilment_method)}</td></tr>)}{!orders?.length&&<tr><td colSpan={5} className="empty">No hamper orders yet.</td></tr>}</tbody></table></div></Shell>
}
