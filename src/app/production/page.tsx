...
function weekBounds(anchor?:string){
  const now=anchor&&/^\d{4}-\d{2}-\d{2}$/.test(anchor)?new Date(`${anchor}T12:00:00`):new Date();
  const day=(now.getDay()+6)%7;
  const start=new Date(now);start.setHours(12,0,0,0);start.setDate(now.getDate()-day);
  ...
  return {start:iso(start),end:iso(end),prev:iso(prev),next:iso(next)};
}
